...
const UNCAUGHT_EXCEPTION_MONITOR_HANDLER_NAME = "uncaughtExceptionMonitor";
...
class AutoCollectExceptions {
    constructor(client) {
        this._canUseUncaughtExceptionMonitor = false;
        this._client = client;
        // Only use for 13.7.0+
        ...
        if (this._canUseUncaughtExceptionMonitor) {
            ...
            this._exceptionListenerHandle = this._handleException.bind(this, false, UNCAUGHT_EXCEPTION_MONITOR_HANDLER_NAME); // never rethrows
            process.on(UNCAUGHT_EXCEPTION_MONITOR_HANDLER_NAME, this._exceptionListenerHandle);
        }
        else {
  ...
  */
    ...
    shutdown() {
        if (this._exceptionListenerHandle) {
            if (this._canUseUncaughtExceptionMonitor) {
                process.removeListener(UNCAUGHT_EXCEPTION_MONITOR_HANDLER_NAME, this._exceptionListenerHandle);
            }
            else {
                if (this._exceptionListenerHandle) {
